...
var (
	...
	openVPNInteropImageDescriptors = map[string]*openVPNInteropImageDescriptor{
		...
		"2.5.11": {
			...
			packageVersion: "2.5.11-0ubuntu0.22.04.3",
...
func requireInteropEnvironmentVersion(t *testing.T, version string) interopEnvironment {
	...
	descriptor.versionOnce.Do(func() {
		versionCheck := runDockerContainerAndWait(t, dockerClient, dockerContainerOptions{
			Name:  "sing-openvpn-interop-version-" + sanitizeDockerName(version+"-"+t.Name()),
		...
		if !hasVersionOutput {
			descriptor.versionErr = E.New("missing OpenVPN version output: ", strings.TrimSpace(versionCheck.Logs))
			return
		}
		if packageVersion != descriptor.packageVersion {
			descriptor.versionErr = E.New("unexpected OpenVPN package version: expected ", descriptor.packageVersion, ", got ", packageVersion)
			return
		}
		...
		if len(versionFields) < 2 || versionFields[0] != "OpenVPN" || versionFields[1] != descriptor.version {
			descriptor.versionErr = E.New("unexpected OpenVPN version: expected ", descriptor.version, ", got ", openVPNVersionLine)
		}
	})
